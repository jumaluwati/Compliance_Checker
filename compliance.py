...
def get_devices_from_dnac(token):
    ...
    while True:
        url = f"https://{DNAC_IP}/dna/intent/api/v1/network-device?family=Switches%20and%20Hubs&limit={limit}&offset={offset}"
        try:
            ...
            if response.status_code == 200:
                ...
                if len(batch) < limit:
                    break  # No more devices left
                offset += limit
            else:
                st.error(f"Failed to fetch devices: {response.status_code} - {response.text}")
                break
        except Exception as e:
...
def main():
    ...
    if token:
        ...
        if devices:
            ...
            select_all = st.checkbox("Select All Switches", key="select_all_switches")
            if select_all:
                selected_dnac_devices = st.multiselect(
                    "Select Devices from DNAC", dnac_device_names, default=dnac_device_names, key="dnac_devices"
                )
            else:
                selected_dnac_devices = st.multiselect(
                    "Select Devices from DNAC", dnac_device_names, key="dnac_devices"
                )
